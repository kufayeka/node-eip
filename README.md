...
| ↳ Connection type: Exclusive-Owner / Input-Only / Listen-Only | ✅ | Diporting dari `appcontype.c` OpENer — klasifikasi berdasarkan slot (O→T, T→O) yang didaftarkan per profil koneksi, bukan dari ukuran O→T. Listen-Only ditolak dengan `0x0119` bila belum ada koneksi "master"; Exclusive-Owner kedua ke T→O yang sama dari originator berbeda ditolak dengan `0x0106` (Ownership Conflict). Memungkinkan PLC memegang koneksi sekaligus HMI/SCADA memantau data yang sama secara bersamaan (PUB00070 §5.2.2/§5.2.3f) |
...
| ↳ Production Trigger: Application Object | ✅ | Diporting dari `TriggerConnections()` OpENer — tidak ada timer otomatis sama sekali; produksi hanya terjadi saat aplikasi memanggil `adapter.triggerProduction(instance)` / `builder.triggerConnection(name)` secara eksplisit, persis definisi trigger ini di CIP Vol 1 Table 3-4.5 |
